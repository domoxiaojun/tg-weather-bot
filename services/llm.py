...
DEFAULT_WEATHER_REPORT_PROMPT = (
    ...
    "5. 全文必须简短：总长度 400-600 字符（含标点），绝不超过 800。宁可少说，不要凑满。\n\n"
    "推荐版式（每块之间空一行，块内不要再分行）：\n"
    "🌤️ <b>现在</b>\n"
    "1-2 句，最多 60 字：温度体感 + 最值得说的 1-2 个数字（湿度/风/空气质量三选一二），其余省略。\n\n"
    "⏱️ <b>接下来</b>\n"
    "1-2 句，最多 60 字：未来 2-6 小时的关键变化。有雨说清开始时间和强度；没雨就一句话带过，不要强行提醒带伞。\n\n"
    "📅 <b>未来几天</b>\n"
    "最多 1 句：只有 daily_forecast 出现明显升降温、雨雪或空气质量转折才写；天气平稳时整块省略。\n\n"
    "👕 <b>建议</b>\n"
    "2-3 条，每条不超过 20 字，写成一行内用「·」分隔；必须基于 life_indices、risk_signals、空气质量和预警，不要罗列所有指数。\n\n"
    "不要写收尾寄语、不要总结、不要鸡汤。\n\n"
    "事实规则：\n"
    "1. 优先依据 risk_signals、预警、分钟级降水、逐小时预报、空气质量和生活指数；不要编造 JSON 没有的数据。\n"
    "1b. 若有 yesterday_observed，可在「现在」块用半句对比昨天（例如比昨天低 3°）；没有该字段就不要提昨天。\n"
    "2. 如果 risk_signals.avoid_unfounded_rain_advice 为 true，不要提醒带伞、洗车会被雨打湿或今晚下雨，除非日预报明确有雨。\n"
    "3. 如果有预警，必须在正文第一块用 ⚠️ <b>预警</b> 单独突出，1 句说清防范要点。\n"
...
class GeminiProvider(LLMProvider):
    ...
    def _extract_text(self, data: dict) -> str:
        try:
            ...
            # 2. OpenAI Format (Proxy Adaptation, non-stream only)
            if "choices" in data:
...
class LLMService:
    ...
    async def generate_weather_report(
        ...
        # Honesty rule: the timestamp must describe the data the BODY was
        # written from. A cache hit therefore renders the cached data_time, not
        # "now" — a 12:00 stamp on conclusions drawn from 08:00 data misleads.
        time_line = f"🕐 数据时间：{data.update_time.strftime('%m月%d日 %H:%M')}"
        ...
        try:
            ...
            if text.strip() and settings.llm_report_cache_ttl_seconds > 0:
                await cache.set(
                    ...
                    {
                        "text": text,
                        "data_time": data.update_time.strftime("%m月%d日 %H:%M"),
                    },
    ...
    @staticmethod
    def _strip_time_header(text: str) -> str:
        """Drop model-emitted timestamp lines; the bot renders the time itself.

        A cached report body must never carry a baked-in "当前时间" — the
        freshness line is prepended per request from live weather data.
        """
        ...
        while lines:
            ...
            if re.match(r"^[🕐⏰\s]*(当前时间|数据时间|更新时间)[：:]", plain):
                lines.pop(0)
                continue
            break
        return "\n".join(lines).strip()
    ...
    def _fix_telegram_html(self, text: str) -> str:
        ...
        # 单星号/下划线限制在行内且紧贴内容，避免误伤 PM2_5、a*b 之类的正文
        text = re.sub(r'(?<!\*)\*([^\s*][^*\n]*?)\*(?!\*)', r'<b>\1</b>', text)
        ...
        # Fix 4: Telegram 会拒绝含不配对标签的整条消息，降级为剥掉该标签。
        text = self._strip_unbalanced_tags(text)

        return text.strip()

    @staticmethod
    def _strip_unbalanced_tags(text: str) -> str:
        for tag in ("b", "i"):
    ...
    @classmethod
    def _truncate_report(cls, text: str, limit: int = 3800) -> str:
        """Keep reports safely under Telegram's 4096-char message limit."""
        if len(text) <= limit:
            return text
        truncated = text[:limit]
        truncated = re.sub(r"<[^>]*$", "", truncated)  # drop a half-cut tag
        truncated = cls._strip_unbalanced_tags(truncated)
