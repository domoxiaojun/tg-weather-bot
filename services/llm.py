...
class LLMService:
    ...
    async def generate_weather_report(self, data: WeatherData) -> str:
        ...
        try:
            ...
            # Auto-correct common Markdown errors for Telegram compatibility
            text = self._fix_telegram_markdown(text)
    ...
    def _build_system_prompt(self) -> str:
        return (
            ...
            "3. **幽默感**：适当调侃天气，或者用可爱的语气，善用各种emoji，但不要过度啰嗦。"
    ...
    def _fix_telegram_markdown(self, text: str) -> str:
        """
        Auto-correct common Markdown errors for Telegram compatibility.
        Fixes:
        - Convert **bold** to *bold* (Telegram Markdown v1 uses single asterisk)
        - Remove stray backticks (unless in code blocks)
        - Escape special characters if needed
        """
        import re
        
        # Fix 1: Replace **text** with *text* (standard Markdown bold → Telegram bold)
        text = re.sub(r'\*\*([^\*]+?)\*\*', r'*\1*', text)
        
        # Fix 2: Remove single backticks (common LLM mistake)
        # Keep triple backticks (code blocks)
        text = re.sub(r'(?<!`)`(?!`)', '', text)
