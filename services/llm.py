...
class LLMService:
    ...
    async def generate_weather_report(self, data: WeatherData) -> str:
        ...
        try:
            ...
            # asyncio超时设为50秒，比httpx的60秒短，让httpx先报错
            ...
            logger.error("LLM Generation Timed Out (50s)")
    ...
    def _build_system_prompt(self) -> str:
        return (
            ...
            "4. **排版美观 - Telegram HTML 格式规则**："
            "   - 加粗：使用 <b>文本</b>，例如：<b>重点内容</b>"
            "   - 斜体：使用 <i>文本</i>，例如：<i>斜体文本</i>"
            "   - ❌ 禁止使用：Markdown符号（*、_、`等）"
            "   - 示例正确格式：今天的温度是 <b>15°C</b>，体感 <i>略冷</i>。"
            "5. **长度控制**：控制在 300 字左右，适合手机快速阅读。"
            "6. 如果有预警信息，必须在最开始使用 <b>加粗</b> 强调。"
            "7. **结尾**：必须包含一句暖心的祝福。"
        )
