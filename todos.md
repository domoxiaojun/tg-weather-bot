...
- [ ] B1 scheduler: job 内 chat_data 修改触发持久化
- [ ] B2 scheduler: 每日简报改为时区感知（默认 Asia/Shanghai，可配置）
- [ ] B3 scheduler: 单地点 wait_for 超时；Forbidden 时清理死订阅；删 job_error_handler 与无效 tenacity retry
- [ ] B4 handlers: `/tq` 频道消息用 effective_message 防 AttributeError；提示去反引号
- [ ] B5 callbacks: 进 handler 先 `query.answer()`
- [ ] B6 report: keep_typing 吞掉全部异常，不毁掉已生成报告
- [ ] B7 inline/formatter: result id 与 callback_data 64 字节安全（短 hash 映射）
- [ ] B8 inline: 删除未使用的 view 参数解析并修正帮助文案
- [ ] B9 subscriptions: 订阅时 geocode 校验 + 归一化存储
- [ ] B10 bot/main: error handler 给用户反馈 + 管理员告警；main.py logger.exception + sys.exit(1)；删 "123456" 校验；post_init 并入 create_app（builder 链式）

## 批次 C — 展示与 LLM 层（visualizer / chart_cache / formatter / llm）

- [ ] C1 chart_cache: 渲染移入单线程 executor（不阻塞事件循环）；失败负缓存
